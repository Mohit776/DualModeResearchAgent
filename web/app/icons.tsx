// icons.tsx — Inline SVG icon components. No emoji, no icon packages.
...
const D = { size: 18, strokeWidth: 1.8 };

function Svg({ size = D.size, strokeWidth = D.strokeWidth, className = "", style, children }: IconProps & { children: React.ReactNode }) {
  return (
    <svg
...
export function IconBulb(p: IconProps) {
  return (
    <Svg {...p}>
      ...
      <path d="M12 2a7 7 0 0 1 7 7c0 3.18-2.12 5.87-5 6.72V18H10v-2.28C7.12 14.87 5 12.18 5 9a7 7 0 0 1 7-7z" />
    </Svg>
  );
}
...
export function IconSpinner({ size = D.size, className = "", style }: IconProps) {
  return (
    <svg
      width={size} height={size} viewBox="0 0 24 24" fill="none"
      className={`animate-spin ${className}`} style={style}
    >
